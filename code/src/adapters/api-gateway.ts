...
export const GetProductAPIGWAdapter =
  (next: GetProduct): APIGatewayProxyHandler =>
  async event => {
...
export const GetProductsAPIGWAdapter =
  (next: GetProducts): APIGatewayProxyHandler =>
  async () => {
    try {
...
export const DeleteProductAPIGWAdapter =
  (next: DeleteProduct): APIGatewayProxyHandler =>
  async event => {
    const id = event.pathParameters!.id;
...
export const CreateProductAPIGWAdapter =
  (next: CreateProduct): APIGatewayProxyHandler =>
  async event => {
    try {
      ...
      if (event.body != null) {
        ...
        return response(
          JSON.stringify({ message: "Product not provided" }),
          400
        );
      }
    } catch (error) {
